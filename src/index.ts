 ...
 *
 * Thousands of US (and global) government bodies publish open GIS data on
 * ArcGIS Hub. `search_datasets` searches the Hub catalog (optionally scoped to
 * one organization via org_id); each result carries a Feature Service `url`.
 * `query_layer` runs a SQL-like query against that layer; `layer_info` returns
 * its schema. Keyless.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Arcgis HUB');
}


const HUB = 'https://hub.arcgis.com/api/v3/datasets';
const UA = 'pipeworx-mcp-arcgis-hub/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'search_datasets',
    description: 'Search the ArcGIS Hub catalog of open government geospatial datasets by keyword. Optionally scope to one organization with org_id (an ArcGIS orgId). Returns each dataset\'s name, summary, record_count, owner/org, and its Feature Service `url` — pass that url to query_layer / layer_info.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Keyword(s), e.g. "parcels", "crime", "flood zones".' },
        org_id: { type: 'string', description: 'Optional ArcGIS orgId to scope results to one publisher (e.g. "RmCCgQtiZLDCtblq" = LA County).' },
